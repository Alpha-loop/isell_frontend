...
function QuoteResultContent({ isNavOpen, setIsNavOpen }) {
  ...
  const origin = searchParams.get('origin') ? decodeURIComponent(searchParams.get('origin')) : 'Unknown';
  const destination = searchParams.get('destination') ? decodeURIComponent(searchParams.get('destination')) : 'Unknown';
  const weightKG = searchParams.get('weightKG') ? decodeURIComponent(searchParams.get('weightKG')) : 'Unknown';
  const quote = searchParams.get('quote') ? parseFloat(decodeURIComponent(searchParams.get('quote'))) : 0;
  const currency = searchParams.get('currency') ? decodeURIComponent(searchParams.get('currency')) : 'USD';
  const estimatedDelivery = searchParams.get('estimatedDelivery') ? decodeURIComponent(searchParams.get('estimatedDelivery')) : 'Unknown';
        ...
        <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 w-full max-w-full box-border">
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between border-t border-gray-300 pt-3">
              <p className="text-sm sm:text-base font-semibold text-gray-500">Quote</p>
              <p className="text-base sm:text-lg font-bold text-gray-900">{currency} {quote.toFixed(2)}</p>
            </div>
            <div className="flex items-center justify-between">
              <p className="text-sm sm:text-base font-semibold text-gray-500">Estimated Delivery</p>
              <p className="text-sm sm:text-base font-bold text-gray-900">{estimatedDelivery}</p>
            </div>
          </div>
          <button
            className="mt-6 w-full bg-green-600 hover:bg-green-700 text-white font-medium py-2 sm:py-3 px-4 rounded-md transition duration-200"
            onClick={() => router.push('/signup')}
          >
            Proceed to Shipping
          </button>
